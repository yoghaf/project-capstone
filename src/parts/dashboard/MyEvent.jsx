          ...
          <div>
            ...
            <Form style={{ fontFamily: "Bold" }}>
              ...
              <Form.Group className="d-flex align-items-center mb-2 mt-2">
                ...
                <Form.Control className="w-100" placeholder="" value={nameEvent} onChange={(v) => setNameEvent(v.target.value)} />
              </Form.Group>
              {/* registration */}
              <Form.Group className="d-flex align-items-center mb-2 mt-2">
                <Form.Label className="w-25">Regitration</Form.Label>
                <div className="d-flex align-items-center flex-grow-1">
                  <Form.Control className="formDate w-45" type="date" value={dateRegisStart == null ? new Date() : dateRegisStart} onChange={(v) => setDateRegisStart(v.target.value)} />
                  <p style={{ marginLeft: 16, marginRight: 16 }}>-</p>
                  <Form.Control className="formDate w-45" type="date" value={dateRegisEnd == null ? new Date() : dateRegisEnd} onChange={(v) => setDateRegisEnd(v.target.value)} />
                </div>
              </Form.Group>
              {/* date */}
              <Form.Group className="d-flex align-items-center mb-2 mt-2">
                ...
                <div className="d-flex align-items-center flex-grow-1">
                  <Form.Control className="formDate w-45" type="date" value={startDate} onChange={(v) => setStartDate(v.target.value)} />
                  <p style={{ marginLeft: 16, marginRight: 16 }}>-</p>
                  <Form.Control className="formDate w-45" type="date" value={endData} onChange={(v) => setEndDate(v.target.value)} />
                </div>
              </Form.Group>
              {/* link image */}
              <Form.Group className="d-flex align-items-center mb-2 mt-2">
                <Form.Label className="w-25">Link image</Form.Label>
                <Form.Control className="w-100" placeholder="" value={link} onChange={(v) => setLink(v.target.value)} />
              </Form.Group>
              {/* description */}
              <Form.Group className="d-flex mb-2 mt-2">
                <Form.Label className="w-25">Description</Form.Label>
                <Form.Control className="w-100" placeholder="" as="textarea" rows={3} value={description} onChange={(v) => setDescription(v.target.value)} />
              </Form.Group>
              {/* city */}
              <Form.Group className="d-flex mb-2 mt-2">
                <Form.Label className="w-25">City</Form.Label>
                <Form.Select className="w-100" placeholder="" value={city} onChange={(v) => setCity(v.target.value)}>
              ...
              <Form.Group className="d-flex mb-2 mt-2">
                ...
                <Form.Control className="w-100" placeholder="" as="textarea" rows={3} value={location} onChange={(v) => setLocation(v.target.value)} />
              </Form.Group>
              {/* Link invite Group */}
              <Form.Group className="d-flex align-items-center mb-2 mt-2">
                <Form.Label className="w-25">Link invite Group</Form.Label>
                <Form.Control className="w-100" placeholder="" value={linkGroup} onChange={(v) => setLinkGroup(v.target.value)} />
              </Form.Group>
              ...
              {/* button save */}
              <Button variant="success" className="w-100 mb-4 mt-2" onClick={() => handleSaveEvent()}>
                SIMPAN
              </Button>{" "}
            </Form>
          </div>
